        ...
        <div x-data="{form: false, fmodal: ''}">
                ...
                <div class="px-4 pt-4">
                    <div class="flex flex-wrap items-center gap-2">
                        <div class="flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2">
                            ...
                            <span class="text-sm font-semibold text-gray-900">{{ $totalLocations  }}</span>
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <table class="w-full text-sm">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 text-gray-900">{{ $location->name }}</td>
                        <td class="px-6 py-4 text-gray-900">{{ $location->campus->name }}</td>
                        <td class="px-6 py-4 text-gray-900">{{ $location->AccessPointAssignment->count() }}</td>
                        <td class="px-6 py-4"><a href="/location/result/{{ $location->id }}" class="text-primary hover:text-primaryDark text-xs font-medium">View →</a></td>
                    </tr>
                    @empty
            ...
            <div x-show="form" x-cloak class="px-2 md:px-0 transition-all duration-300 flex h-screen w-full bg-black/20 fixed top-0 left-0 z-50  justify-center items-center">
                <div @click.away="form = false" class=" rounded-lg">
                    <livewire:ap_assignment.create_location/>
                </div>
            </div>
        </div>
